 ...
 *
 * The 68060 is always serialized (MC68060UM 7.10), and the CT60 TOS maps
 * 0x80000000-0xffffffff cache inhibited precise (ct60tos pmmu2.c), which
 * is the IOMAP_NOCACHE_SER mapping ioremap() gives Linux. Every access
 * below therefore completes on the bus in program order, and the
 * descriptor write that hands a buffer to the MAC cannot overtake the
...
static long ethoc_probe(struct netif *netdev, char *membase, const u8 *hwaddr)
{
	...
	/*
	 * Not in Linux, where the MAC comes up fresh: the MAC may still be
	 * running from before a warm boot, and must not raise anything
	 * before the vector is ours.
	 */
